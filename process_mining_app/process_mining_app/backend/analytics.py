...
def _build_nodes(df: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    event_durations = pd.DataFrame(
        {
            ACTIVITY_COLUMN: df[ACTIVITY_COLUMN],
            "duration_seconds": (
                _event_end_series(df) - _event_start_series(df)
            ).dt.total_seconds().fillna(0).clip(lower=0),
        }
    )
    total_activity_durations = event_durations.groupby(
        ACTIVITY_COLUMN,
        observed=True,
        ...
        nodes.append(
            {
                ...
                "total_duration_seconds": round(
                    _safe_float(total_activity_durations.get(activity, 0)),
                    2,
                ),
            }
        )
...
def _build_edges_payload(edges_df: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    result = edges_df[["source", "target", "frequency", "total_duration_seconds",
                        "mean_duration_seconds", "median_duration_seconds",
                        "p90_duration_seconds"]].copy()
...
def _build_swimlane_view(
    ...
    """Build a normalized BPMN-style process-flow model.

    The frontend still reads this from the historical ``swimlane`` key, but the
    data is no longer actor-lane based. Activities are normalized to one node
    each, and branching/merging metadata lets the UI render a traditional
    flowchart with BPMN task boxes and gateway diamonds.
    """
    if df.empty:
        return {
            "available": False,
            "view_type": "bpmn_flowchart",
            "normalized_activities": True,
            "actor_column": detect_actor_column(df),
            "lanes": [],
            "nodes": [],
            "edges": [],
    ...
    ordered = df.sort_values([CASE_COLUMN, TIMESTAMP_COLUMN]).copy()
    ordered["position"] = ordered.groupby(CASE_COLUMN, observed=True).cumcount()
    total_cases = max(int(ordered[CASE_COLUMN].nunique()), 1)
    starts = ordered.groupby(CASE_COLUMN, observed=True).first()[ACTIVITY_COLUMN].value_counts()
    ends = ordered.groupby(CASE_COLUMN, observed=True).last()[ACTIVITY_COLUMN].value_counts()
    median_positions = ordered.groupby(ACTIVITY_COLUMN, observed=True)["position"].median()
    cases_per_activity = ordered.groupby(ACTIVITY_COLUMN, observed=True)[CASE_COLUMN].nunique()
    activity_counts = ordered[ACTIVITY_COLUMN].value_counts()
    event_durations = pd.DataFrame(
        {
            ACTIVITY_COLUMN: ordered[ACTIVITY_COLUMN],
            "duration_seconds": (
                _event_end_series(ordered) - _event_start_series(ordered)
            ).dt.total_seconds().fillna(0).clip(lower=0),
        }
    )
    total_activity_durations = event_durations.groupby(
    ...
    activities = sorted(
        activity_counts.index.astype(str).tolist(),
        key=lambda activity: (
            _safe_float(median_positions.get(activity, 0)),
            -int(activity_counts.get(activity, 0)),
            activity,
        ),
    )
    nodes = [
        {
            ...
            "case_frequency": int(cases_per_activity.get(activity, 0)),
            "case_coverage": round(
                float(cases_per_activity.get(activity, 0) / total_cases),
                4,
            ),
            "start_count": int(starts.get(activity, 0)),
            "end_count": int(ends.get(activity, 0)),
            "median_position": round(_safe_float(median_positions.get(activity, 0)), 2),
            "total_duration_seconds": round(
                _safe_float(total_activity_durations.get(activity, 0)),
                2,
            ),
    ...
    flow_transitions = transitions if transitions is not None else _transition_dataframe(df)
    if flow_transitions.empty:
        return {
    ...
    edges_df = (
        flow_transitions.groupby(["source", "target"], observed=True)
        .agg(
            frequency=(CASE_COLUMN, "size"),
            case_frequency=(CASE_COLUMN, "nunique"),
            ...
            p90_duration_seconds=("duration_seconds", lambda series: float(series.quantile(0.9))),
        )
        .reset_index()
        .sort_values("frequency", ascending=False)
    )
    ...
    outgoing_totals = edges_df.groupby("source", observed=True)["frequency"].sum()
    incoming_totals = edges_df.groupby("target", observed=True)["frequency"].sum()
    outgoing_degree = edges_df.groupby("source", observed=True)["target"].nunique()
    incoming_degree = edges_df.groupby("target", observed=True)["source"].nunique()
    ...
    edges = [
        {
            ...
            "outgoing_share": round(
                float(row["frequency"] / max(outgoing_totals.get(row["source"], 0), 1)),
                4,
            ),
            "incoming_share": round(
                float(row["frequency"] / max(incoming_totals.get(row["target"], 0), 1)),
                4,
            ),
            "total_duration_seconds": round(
                _safe_float(row["total_duration_seconds"]),
                2,
            ),
            "median_duration_seconds": round(
                _safe_float(row["median_duration_seconds"]),
                2,
            ),
            "mean_duration_seconds": round(
                _safe_float(row["mean_duration_seconds"]),
                2,
            ),
            "p90_duration_seconds": round(
                _safe_float(row["p90_duration_seconds"]),
                2,
            ),
        }
        for _, row in edges_df.head(420).iterrows()
    ]
    ...
    gateways = [
        {
            ...
            "gateway_type": "split",
            "degree": int(degree),
            "frequency": int(outgoing_totals.get(activity, 0)),
        }
        for activity, degree in outgoing_degree.items()
        if int(degree) > 1
    ] + [
        {
            "id": f"merge::{activity}",
            ...
            "gateway_type": "merge",
            "degree": int(degree),
            "frequency": int(incoming_totals.get(activity, 0)),
        }
        for activity, degree in incoming_degree.items()
        if int(degree) > 1
    ]
...
def _mermaid_bpmn_flowchart(flowchart: dict[str, Any]) -> str | None:
    """Serialize the normalized BPMN-style flowchart as Mermaid source."""
    if not flowchart.get("available"):
        return None
    ...
    for edge in flowchart.get("start_edges", [])[:80]:
        target_alias = alias_by_id.get(str(edge.get("target", "")))
        if target_alias:
            lines.append(f'START -->|{edge.get("frequency", "")}| {target_alias}')
    ...
    for edge in flowchart.get("end_edges", [])[:80]:
        source_alias = alias_by_id.get(str(edge.get("source", "")))
        if source_alias:
            lines.append(f'{source_alias} -->|{edge.get("frequency", "")}| END')
...
# ---------------------------------------------------------------------------
# Disco-compatible CSV exports
# ---------------------------------------------------------------------------

DISCO_ACTIVITY_EXPORT_COLUMNS = [
    "Activity",
    "Absolute frequency",
    "Case frequency",
    "Max. repetitions",
...
DISCO_RELATION_EXPORTS = {
    "Relations - Absolute Frequency.csv": ("absolute_frequency", True),
    "Relations - Case Frequency.csv": ("case_frequency", True),
    "Relations - Case Coverage.csv": ("case_coverage", False),
    "Relations - Maximum Repetition.csv": ("maximum_repetition", True),
    "Relations - Total Duration.csv": ("total_duration_ms", True),
    "Relations - Median Duration.csv": ("median_duration_ms", True),
    "Relations - Mean Duration.csv": ("mean_duration_ms", True),
    "Relations - Minimum Duration.csv": ("min_duration_ms", True),
    "Relations - Maximum Duration.csv": ("max_duration_ms", True),
}


def _disco_activity_order(df: pd.DataFrame) -> list[str]:
    """Order activities in a stable process-flow sequence for matrix exports."""
    if df.empty:
        return []

    ordered = df.sort_values([CASE_COLUMN, TIMESTAMP_COLUMN]).copy()
    ordered["_case_position"] = ordered.groupby(CASE_COLUMN, observed=True).cumcount()
    ordered["_seen_order"] = range(len(ordered))

    median_positions = ordered.groupby(ACTIVITY_COLUMN, observed=True)[
        "_case_position"
    ].median()
    first_seen = ordered.groupby(ACTIVITY_COLUMN, observed=True)["_seen_order"].min()
    frequencies = ordered[ACTIVITY_COLUMN].value_counts()
    activities = ordered[ACTIVITY_COLUMN].dropna().astype(str).unique().tolist()

    # Median case position gives a left-to-right process order. First-seen order
    # keeps ties intuitive for logs where several activities occupy the same
    # median position.
    return sorted(
        activities,
        key=lambda activity: (
            _safe_float(median_positions.get(activity, 0)),
            int(first_seen.get(activity, 0)),
            -int(frequencies.get(activity, 0)),
            str(activity),
        ),
    )


def _local_zoneinfo_for_disco_exports() -> Any:
    """Find the machine timezone for desktop-style Disco duration semantics."""
    candidates: list[str] = []
...
def _disco_clock_series(series: pd.Series) -> pd.Series:
    """Interpret timestamp clock values in local time for Disco-like durations.

    FlowScope normalizes timestamps to UTC for internal analytics. Fluxicon
    Disco's CSV duration exports behave like a desktop app: naive timestamps are
    interpreted in the local machine timezone, so intervals crossing daylight
    saving changes shrink or expand accordingly. This helper recreates that
    behavior without changing the app's canonical timestamp columns.
    """
    parsed = pd.to_datetime(series, errors="coerce", utc=True)
    local_tz = _local_zoneinfo_for_disco_exports()
    if local_tz is None:
        return parsed

    clock_values = parsed.dt.tz_localize(None)
    localized = clock_values.dt.tz_localize(
        local_tz,
        ambiguous=False,
        nonexistent="NaT",
    )
...
def _duration_milliseconds_between(starts: pd.Series, ends: pd.Series) -> pd.Series:
    """Return non-negative local-time durations in milliseconds."""
    raw_start_times = pd.to_datetime(starts, errors="coerce", utc=True)
    raw_end_times = pd.to_datetime(ends, errors="coerce", utc=True)
    start_times = _disco_clock_series(starts)
    end_times = _disco_clock_series(ends)
    local_durations = (end_times - start_times).dt.total_seconds()
    raw_durations = (raw_end_times - raw_start_times).dt.total_seconds()
    # Nonexistent local clock values occur during the spring DST gap. Disco
    # leaves those individual clock differences intact, while valid intervals
    # crossing the DST boundary reflect the local offset change.
    durations = local_durations.where(
        start_times.notna() & end_times.notna(),
        raw_durations,
    )
    return durations.fillna(0).clip(lower=0).mul(1000).round().astype("int64")


def _event_duration_milliseconds(df: pd.DataFrame) -> pd.Series:
    """Return event execution durations in milliseconds for Disco-style tables."""
    return _duration_milliseconds_between(_event_start_series(df), _event_end_series(df))


def _disco_activities_csv(df: pd.DataFrame, activity_order: list[str]) -> str:
    """Create the Disco-style ``Activities.csv`` export body."""
    if df.empty:
        return pd.DataFrame(columns=DISCO_ACTIVITY_EXPORT_COLUMNS).to_csv(index=False)

    working = pd.DataFrame(
        {
            ACTIVITY_COLUMN: df[ACTIVITY_COLUMN].astype(str),
            CASE_COLUMN: df[CASE_COLUMN].astype(str),
            "_duration_ms": _event_duration_milliseconds(df),
        }
    )
    grouped = working.groupby(ACTIVITY_COLUMN, observed=True)
    stats = grouped.agg(
        absolute_frequency=(CASE_COLUMN, "size"),
        case_frequency=(CASE_COLUMN, "nunique"),
        total_duration_ms=("_duration_ms", "sum"),
        median_duration_ms=("_duration_ms", "median"),
        mean_duration_ms=("_duration_ms", "mean"),
        min_duration_ms=("_duration_ms", "min"),
        max_duration_ms=("_duration_ms", "max"),
    )

    activity_case_counts = working.groupby(
        [ACTIVITY_COLUMN, CASE_COLUMN], observed=True
    ).size()
    stats["max_repetitions"] = activity_case_counts.groupby(
        level=ACTIVITY_COLUMN, observed=True
    ).max()
    total_cases = max(working[CASE_COLUMN].nunique(), 1)
    stats["case_coverage"] = stats["case_frequency"] / total_cases

    stats = stats.reindex(activity_order).fillna(0)
    result = pd.DataFrame(
        {
            "Activity": activity_order,
            "Absolute frequency": stats["absolute_frequency"].astype(int).values,
            "Case frequency": stats["case_frequency"].astype(int).values,
            "Max. repetitions": stats["max_repetitions"].astype(int).values,
            "Case coverage": stats["case_coverage"].astype(float).values,
            "Total duration (ms)": stats["total_duration_ms"].astype(int).values,
            "Median duration (ms)": stats["median_duration_ms"].astype(int).values,
            "Mean duration (ms)": stats["mean_duration_ms"].astype(int).values,
            "Min. duration (ms)": stats["min_duration_ms"].astype(int).values,
            "Max. duration (ms)": stats["max_duration_ms"].astype(int).values,
        }
    )
    return result[DISCO_ACTIVITY_EXPORT_COLUMNS].to_csv(index=False)


def _disco_relation_stats(df: pd.DataFrame) -> pd.DataFrame:
    """Aggregate direct-follow relations for the Disco-style matrix exports."""
    columns = [
        "source",
        "target",
        "absolute_frequency",
    ...
    transitions = transitions.copy()
    transitions["source"] = transitions["source"].astype(str)
    transitions["target"] = transitions["target"].astype(str)
    transitions["_duration_ms"] = _duration_milliseconds_between(
        transitions["source_timestamp"],
        transitions["transition_time"],
    )

    grouped = transitions.groupby(["source", "target"], observed=True)
    stats = grouped.agg(
        absolute_frequency=(CASE_COLUMN, "size"),
        case_frequency=(CASE_COLUMN, "nunique"),
        total_duration_ms=("_duration_ms", "sum"),
    ...
    per_case_relation_counts = transitions.groupby(
        [CASE_COLUMN, "source", "target"], observed=True
    ).size()
    maximum_repetition = (
        per_case_relation_counts.groupby(["source", "target"], observed=True)
        .max()
        .reset_index(name="maximum_repetition")
    )
    stats = stats.merge(maximum_repetition, on=["source", "target"], how="left")
    total_cases = max(transitions[CASE_COLUMN].nunique(), 1)
    stats["case_coverage"] = stats["case_frequency"] / total_cases
...
def _disco_relation_matrix_csv(
    ...
    *,
    ...
    """Serialize one relation metric as a Disco-style adjacency matrix.

    Disco writes target activities as rows and source activities as columns.
    Keeping that orientation makes these CSVs easy to compare against Disco's
    own relation exports for the same log.
    """
    matrix = pd.DataFrame(0.0, index=activity_order, columns=activity_order)
    matrix.index.name = ""

    if not relation_stats.empty:
        for row in relation_stats.itertuples(index=False):
            source = str(row.source)
            target = str(row.target)
            if source not in matrix.columns or target not in matrix.index:
                continue
            matrix.at[target, source] = getattr(row, metric_column)
...
@_timed("disco_csv_export_payload")
def disco_csv_export_payload(df: pd.DataFrame) -> dict[str, str]:
    """Build CSV files that mirror Fluxicon Disco's activity/relation exports."""
    activity_order = _disco_activity_order(df)
    relation_stats = _disco_relation_stats(df)
    exports = {
        "Activities.csv": _disco_activities_csv(df, activity_order),
    }

    for filename, (metric_column, integer_values) in DISCO_RELATION_EXPORTS.items():
        exports[filename] = _disco_relation_matrix_csv(
            activity_order,
            relation_stats,
            metric_column,
...
def mermaid_export_payload(dashboard: dict[str, Any]) -> dict[str, str]:
    ...
    bpmn_flowchart_mermaid = _mermaid_bpmn_flowchart(dashboard.get("swimlane", {}))
    if bpmn_flowchart_mermaid:
        mermaid["bpmn_flowchart"] = bpmn_flowchart_mermaid
