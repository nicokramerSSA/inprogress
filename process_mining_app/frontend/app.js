...
const FLOW_COLORS = {
  edgeMarker: "#1788ff",
  anchorStroke: (alpha) => `rgba(23, 136, 255, ${alpha})`,
  backboneStroke: (alpha) => `rgba(0, 102, 255, ${alpha})`,
  secondaryStroke: (alpha) => `rgba(62, 168, 255, ${alpha})`,
  activeStroke: (alpha) => `rgba(0, 191, 255, ${alpha})`,
  caseBallFill: "#ffea3d",
  caseBallStroke: "rgba(0, 0, 0, 0.56)",
};
...
function renderMapSelectionPanel() {
  ...
  const selectionVisible = ["process", "handoff_actor", "handoff_activity"].includes(
    state.currentView
  );
...
function renderProcessMap(nodes, edges) {
  ...
  simplified.edges.slice(0, 260).forEach((edge, index) => {
    ...
    let strokeWidth = 1.2 + strength * 10 + (isBackbone ? 1.4 : 0);
    let strokeColor = isBackbone
      ? FLOW_COLORS.backboneStroke(0.64 + strength * 0.32)
      : FLOW_COLORS.secondaryStroke(0.5 + strength * 0.32);
    let opacity = isBackbone ? 0.98 : 0.92;
...
function renderSwimlaneDiagram(swimlane) {
  // Swimlane is intentionally separate from the staged process-map layout: it
  // emphasizes who performed work, not just which activities followed which.
  els.processMap.innerHTML = "";
  ...
  swimlane.lanes.forEach((lane, laneIndex) => {
    ...
    laneLayer.appendChild(
      svgElement("rect", {
        ...
        height: laneHeight - 4,
        rx: 10,
        fill: laneIndex % 2 === 0 ? "rgba(0,51,153,0.06)" : "rgba(0,51,153,0.03)",
  ...
  (swimlane.edges || []).slice(0, 260).forEach((edge, index) => {
    ...
    edgeLayer.appendChild(
      svgElement("path", {
        ...
        fill: "none",
        stroke: FLOW_COLORS.secondaryStroke(0.46 + strength * 0.34),
        "stroke-width": 1 + strength * 8,
        opacity: 0.76 + strength * 0.18,
      })
    );
  });
