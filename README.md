...
- Export package:
  ...
  - `swimlane_nodes.csv`, `swimlane_edges.csv`, `sankey_links.csv`
  - `rework_activity.csv`, `rework_self_loops.csv`
  - `informational_columns_profile.json`
  - `filtered_events.csv`
  - `animation_frames.json`
  - Mermaid diagrams (`*.mmd`) for process map, handoff views, swimlane, sankey, and rework (when available)
  - BPMN model export (`process_model.bpmn`) when supported by your pm4py version
- Self-contained HTML report export:
  - Single downloadable `.html` file with summary metrics, process map snapshot, handoff/swimlane/sankey/rework sections, Mermaid blocks, and BPMN XML
...
Actor/resource-aware features (handoff and swimlane) are enabled automatically when one of these columns is present and populated:
...
- The analytics layer computes activity transitions once per dashboard request and shares the result across all sub-computations (edges, handoffs, swimlane, sankey, rework), avoiding redundant passes over the event log.
