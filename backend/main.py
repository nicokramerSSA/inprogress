...
def _build_html_report(
    *,
    ...
    swimlane = dashboard.get("swimlane", {})
    ...
    swimlane_table = _html_table(
        ["Actor Lane", "Activity", "Frequency"],
        [
            [
                escape(str(node.get("actor", ""))),
                escape(str(node.get("activity", node.get("label", "")))),
                _format_int(node.get("frequency")),
            ]
            for node in swimlane.get("nodes", [])
        ],
    )
    ...
    return (
        ...
        + "</section>"
        "<section class=\"card\"><h2>Swimlane View Data</h2>"
        + swimlane_table
        + "</section>"
